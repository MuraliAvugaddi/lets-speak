// app/courses/[language]/mode/[mode]/clubs/[club]/ClubDetailPageClient.tsx
"use client";
...
export default function ClubDetailPageClient() {
  ...
  if (!courseData) return <div className={styles.errorContainer}>Course not found</div>;

  const selectedMode = courseData.modes.find((m) => m.id === mode);
  if (!selectedMode) return <div className={styles.errorContainer}>Mode not found</div>;

  const clubs = (selectedMode as any).clubs || selectedMode.stages;
  const clubData = clubs.find((c: any) => c.id === club);
  if (!clubData) return <div className={styles.errorContainer}>Club not found</div>;
const getPlanner = () => {
